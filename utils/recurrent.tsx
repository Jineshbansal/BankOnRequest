...
export const recurrent= async () => {
  ...
  const signer = new ethers.Wallet(process.env.NEXT_PUBLIC_SMART_CONTRACT_ADDRESS, provider);
  const contract = new ethers.Contract(contractAddress, contractABI, signer);

  const collateral="0x0";
  const tokenAddress="0x0";
  const data = await contract.shouldCreateRequest(user, collateral,tokenAddress);
  console.log(data);
