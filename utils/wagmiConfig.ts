...
export const onboardConfig = {
  ...
  appMetadata: {
    name: 'BankOnRequest',
    icon: 'public/exchange.png',
    description: 'BankOnRequest is a banking app',
  },
};

const rpcUrls = {
  polygon: process.env.NEXT_PUBLIC_RPC_URL_POLYGON || 'https://1rpc.io/matic',
  ethereum:
    process.env.NEXT_PUBLIC_RPC_URL_ETHEREUM || 'https://eth.llamarpc.com',
  sepolia:
    process.env.NEXT_PUBLIC_RPC_URL_SEPOLIA || 'https://sepolia.drpc.org',
...
const transport = {
  polygon: http(rpcUrls.polygon),
  ethereum: http(rpcUrls.ethereum),
  sepolia: http(rpcUrls.sepolia),
};

export const web3OnboardConfig = {
  onboardConfig,
  rpcUrls: transport,
